...
Homogeneous_crossover::Homogeneous_crossover (double prob):
CR(prob)
{
	in_data.resize(2);
	out_data.resize(2);
...
// DE crossover (in original article of Storn and Price) synchronous variant:
syn_DE_crossover::syn_DE_crossover (int global, int start, int local):
global_dim(global), start_dim(start), local_dim(local)
{
	in_data.resize(2);
	out_data.resize(1);
	for (int i = 0; i < in_data.size(); ++i)
...
void syn_DE_crossover::apply ()
{
	...
	{
		rnd_num[0] = Randomizer::integer_rand(0, global_dim - 1);
		rnd_num[1] = Randomizer::integer_rand(1, global_dim);
	}
	Processors::bcast_from_root(rnd_num, 2*sizeof(int));
	int start = rnd_num[0];
	bool cycle = (start + rnd_num[1] >= global_dim);
	int fin = (start + rnd_num[1]) % global_dim;
	for (int i = 0; i < local_dim; ++i)
	{
		int idx = start_dim + i;
		if (!cycle && (idx >= start && idx < fin) || cycle && (idx >= start || idx < fin))
		{
			out_data[0] -> set_gene(i, in_data[1] -> get_gene(i));
		}
	}
}
void syn_DE_crossover::print (ostream& out)
{
	out << "DE crossover (invented by Price and Storn) synchronous variant:\n";
	out << "\tIn data size: " << in_data.size() << endl;
	out << "\tOut data size: " << out_data.size() << endl;
}
