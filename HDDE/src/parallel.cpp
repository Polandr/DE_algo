...
void Processors::rotation_exchange_doubles (int rot_step, double* data, int num)
{
	int src, dst;
	shift(rot_step, &src, &dst);
	int send_tag, recv_tag;
	send_tag = recv_tag = DBL_EXCH_TAG;
	MPI_Sendrecv_replace(data, num, MPI_DOUBLE, dst, send_tag, src, recv_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
...
double Processors::sum_reduction (double val)
{
	double out;
...
int Processors::start_partition (int num, int proc)
{
	if (num < 0)
		return -1;
	int out = num / proc_num * proc;
	if (proc < num % proc_num)
		out += proc;
	else
		out += num % proc_num;
	return out;
...
int Processors::partition (int num, int proc)
{
	if (num < 0)
		return -1;
	int out = num / proc_num;
	if (proc < num % proc_num)
		out++;
	return out;
}
